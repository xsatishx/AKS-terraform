...
output "aks_config" { value = azurerm_kubernetes_cluster.dev-aks-cluster.kube_config }

# Required to set access policy on key vault.
output "aks_uai_agentpool_object_id" { value = azurerm_kubernetes_cluster.dev-aks-cluster.kubelet_identity[0].object_id }

# Required when setting up csi driver secret provier class.
output "aks_uai_agentpool_client_id" { value = azurerm_kubernetes_cluster.dev-aks-cluster.kubelet_identity[0].client_id }